...
use chrono::Local;
use chrono::Timelike;
use egui::{pos2, Color32, Painter, Rect, RichText, Stroke, Ui};
...
impl Utils {
    ...
    pub fn hex_color(hex: &str) -> Color32 {
        Color32::from_hex(hex).expect("Invalid HEX Color Code!")
    }
    ...
    pub fn screen_reader_switch(ui: &mut Ui, state: &mut bool) {
    ...
    pub fn project_card(&self, ui: &mut Ui, title: &str, desc: &str, url: &str) {
        //TODO: make the card clickable.
        let response = ui
            .group(|ui| {
                ui.set_max_width(200.0);
                ui.vertical(|ui| {
                    ui.heading(RichText::new(title).color(Color32::from_hex("#CB2D3E").unwrap()));
    ...
    pub fn draw_grid(&self, painter: &Painter, rect: Rect) {
        ...
        let mut x = min.x;
        while x <= max.x {
            painter.line_segment([pos2(x, min.y), pos2(x, max.y)], Stroke::new(1.0, color));
            x += grid_size;
        }

        let mut y = min.y;
        while y <= max.y {
            painter.line_segment([pos2(min.x, y), pos2(max.x, y)], Stroke::new(1.0, color));
            y += grid_size;
        }
    }
}
